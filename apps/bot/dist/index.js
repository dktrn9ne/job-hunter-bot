...
app.post('/jobs/ingest', (req, res) => {
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `).run(jobId, ts, ts, parsed.data.sourceUrl || null, parsed.data.platform, parsed.data.company || null, parsed.data.title || null, parsed.data.location || null, parsed.data.jdText, 'NEW');
...
app.post('/jobs/:id/generate', async (req, res) => {
    ...
    if (!row)
        return res.status(404).json({ ok: false, error: 'not_found' });
    if (!env.CHROME_PATH)
        return res.status(500).json({ ok: false, error: 'CHROME_PATH_missing' });
...
app.post('/apply/email', async (req, res) => {
    ...
    if (!parsed.success)
        return res.status(400).json({ ok: false, error: parsed.error.flatten() });
    const job = db.prepare('SELECT * FROM jobs WHERE id=?').get(parsed.data.jobId);
    if (!job)
        return res.status(404).json({ ok: false, error: 'job_not_found' });
    const doc = db.prepare('SELECT * FROM documents WHERE job_id=? ORDER BY created_at DESC LIMIT 1').get(parsed.data.jobId);
    if (!doc)
        return res.status(400).json({ ok: false, error: 'no_documents_generated' });
    if (!env.SENDGRID_API_KEY)
        return res.status(500).json({ ok: false, error: 'SENDGRID_API_KEY_missing' });
    if (!env.MAIL_FROM)
        return res.status(500).json({ ok: false, error: 'MAIL_FROM_missing' });
